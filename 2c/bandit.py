...
class Bandit:
    def __init__(self, variance, size, stationary):
        self.true_value = np.random.normal(loc = 0, scale = 1)
        self.rewards = np.random.normal(loc = self.true_value, scale = 1, size = 100)
        self.stationary = stationary
        self.size = size

    def get_reward(self):
        #return np.random.normal(loc = self.true_value, scale = 1)
        return np.random.choice(self.rewards)

class KBanditProblem:

    def __init__(self, k_arms, variance, size, stationary, steps, step_size_parameter):
        self.k_arms = k_arms
        self.steps = steps
        ...
        self.epsilon = step_size_parameter
        self.reset()
    ...
    def update_values_stationary(self):
        if not self.stationary:
            values = np.random.normal(loc = 0, scale = 0.01, size = self.k_arms)
            for i, bandit in zip(list(range(self.k_arms)), self.bandits):
...
if __name__ == "__main__":
    ...
    # stationary
    bandit = KBanditSolution(k, variance, size, stationary, steps, step_size_parameter)
